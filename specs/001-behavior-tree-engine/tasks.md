...
- [X] T022 [US1] Rewire `shared-rs/consensus/src/praos.rs` (block/tip paths) to drop `on_block_received`/`on_tip_advanced` hook calls; reorg/drop read from `ControlSignal`; test.
...
- [X] T024 [US1] Rewire production: `shared-rs/consensus/src/production.rs` body-path and `net-rs/net-node/src/production.rs` to read `praos.production`/`praos.body_path` from the published `ControlSignal` (remove `rb_production_strategy`/`decide_body_path` hooks); test.
- [X] T024s [US1] **(Scope amendment, 2026-06-22)** Migrate **sim-rs** onto the BT engine BEFORE T025. `shared-consensus` is shared by net-node, net-cluster AND sim-rs; sim-rs was still on the `Behaviour` hook trait (no BT wiring), so T021/T023 left its hook-based lazy-voter/t22 dormant and T025 would break its build. Wire a per-node BT runtime + `apply_control` into sim-rs's per-slot loop, give it a BT-config surface (replacing `ConsensusBehaviourEntry`/`BehaviourSpec`/`resolve_specs`), and drop its hook usage. User decision: migrate sim-rs first, then delete. See [[bt-engine-shared-across-net-and-sim]].
- [X] T025 [US1] Delete the `Behaviour` hook trait and all its hooks (incl. the merged `allow_echo_to_source`/`transform_outbound`), `BehaviourOutcome`/`DecisionOutcome`, `CompositeBehaviour`, `invoke_hook`, the `Outbound`/`OwnedOutbound` enums, and the old per-behaviour `impl Behaviour` blocks from `shared-rs/consensus/src/behaviour/`; collapse `registry.rs` to `ActionSpec` + `build_action` only; ensure **`shared-rs` + `net-rs` + `sim-rs`** compile and all tests pass. **(Now unblocked only after T024s.)**
- [X] T026 [US1] Create `net-rs/net-node/src/bt_runtime.rs`: holds the `DynamicEnv` + `BehaviourTree` and ticks once per slot. **Done: runtime + honest default + load/compile + 3 tests. The published `ControlSignal` snapshot + variant store are added with the net-core actuator rewire (T029).**
- [X] T027 [US1] Wire the tick into `net-rs/net-node/src/main.rs` slot arm: build `NativeChainState`, tick the BT, `apply_control` to the state machines. **Done: tick + `Consensus::apply_control` fan-out (leios+praos) wired at the top of the slot arm — vote + t22 now driven by the BT. Pending: snapshot publication; cut over reorg/drop/production reads from the old handle to the signal; remove the stdin `behaviour`/`behaviour_reset` handling (with T025). Epoch/mempool-count are placeholder 0 (TODO) — MVP slot-trigger reads only current_slot.**
- [X] T028 [US1] Add `--behaviour-tree <path>` (and a `behaviour_tree` config key) to `net-rs/net-node/src/config.rs`; load + compile the `BtConfig`; default to an implicit honest one-leaf tree when absent; test (incl. start-up refusal on an invalid config, US1 scenario 5). **Done. Removal of the legacy `behaviour`/`behaviour_reset` config fields happens with the T025 deletion.**
- [X] T029 [US1] Rewire the RB-header per-peer outbound actuator in `net-rs/net-core/src/peer/server_handlers.rs` to read the published `praos.outbound` (equivocation routing / drop) instead of `transform_outbound`; test.
- [X] T029a [US1] Rewire the LeiosNotify offer actuator `serve_leios_notify` in `net-rs/net-core/src/peer/server_handlers.rs` to read the published `leios.offer_eb_size` (rewrite `eb_size`) and `leios.echo_to_source` (no-echo gate) per offer entry, instead of `allow_echo_to_source`/`transform_outbound`; preserve the `NotificationEntry { source, eb_size }` substrate; port the merged 3 mux-pair integration tests.
...
- [X] T031 [US1] [US4] Config split in `net-rs/net-cluster/`: extract each `[behaviour]`/`[behaviour_selection]` block from `configs/*.toml` into named BT configs under `net-cluster/behaviours/`; reference them by name from the (topology-only) configs; update `src/config.rs` + `src/process.rs` to distribute the referenced BT config to the selected nodes at spawn (keep a deprecation shim for inline `[behaviour]`); tests for load + round-trip.
- [X] T032 [P] [US1] Author sample BT configs under `net-rs/net-cluster/behaviours/` (honest + the re-homed catalogue), including the duplex-follower-bug `Join["echo-to-source", lie-about-eb-size(0,1,0)]` example, and a single-node `--behaviour-tree` example, matching `contracts/bt-config.schema.md`.
